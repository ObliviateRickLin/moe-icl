"""
Conformal prediction diagnostics for mix_noise2 ICL experiments.
...
We use split conformal for each ICL length L independently:
  - Evaluate once at max_n_points (default 21) to get per-position predictions.
...
Outputs (under results/_icl_curves/):
  - compare_mix_noise2_conformal_<family>_<task>_alpha0p05.png
  - compare_mix_noise2_conformal_summary_<family>_alpha0p05.csv
"""
...
def _conformal_q(scores: torch.Tensor, alpha: float) -> float:
    ...
    k = max(1, min(k, n))
    # kth order statistic (1-indexed): sort and take k-1
...
def main():
    ...
    parser.add_argument("--max-n-points", type=int, default=21)
    ...
    for exp in exp_list:
        ...
        for task in tasks:
            ...
            n_cal = int(round(args.calib_frac * n_total))
    ...
    csv_path = out_dir / f"compare_mix_noise2_conformal_summary_{args.family}_alpha{alpha_tag}.csv"
    if rows:
    ...
    # Plot one figure per task label (noise=0.1 / 0.5).
    ...
    for task_label, per_exp in plots.items():
        ...
        out_png = out_dir / f"compare_mix_noise2_conformal_{args.family}_{safe}_alpha{alpha_tag}.png"
