...
class EncoderTransformer(nn.Module):
    ...
    def forward(self, xs, ys, inds=None, return_hidden_states=False, return_aux_loss=False):
        if inds is None:
        ...
        output = prediction[:, inds, 0]
        
        if return_hidden_states and return_aux_loss:
            return output, hidden_states, total_aux_loss
...
class GPT2MoEWrapper(nn.Module):
    def __init__(
        self,
        n_embd,
        num_experts=4,
...
class TransformerModel(nn.Module):
    ...
    def forward(self, xs, ys, inds=None, return_aux_loss: bool = False):
        ...
        out = prediction[:, ::2, 0][:, inds]  # predict only on xs
        if return_aux_loss:
